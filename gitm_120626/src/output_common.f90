...
subroutine output(dir, iBlock, iOutputType)
  ...
  case ('3DALL')
!!modified by MYZ AUG-08-2019
           open(unit=iOutputUnit_+100, form="unformatted", &
                file=dir//"/trans_"//cTime(1:cL)//".DAT",&
                status="replace")
...
!!modified by MYZ AUG-08-2019
 ...
 case ('1DTHM')
  ...
  !!modified by MYZ, for remove the 3dfiles, since we only output the upper boundary of GITM
  close (unit=iOutputUnit_, status='delete')
  close(iOutputUnit_+100)
